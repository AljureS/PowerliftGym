import styles from "../../components/Turno/Turno.module.css"
import logoTurn from "../../assets/logoTurn.png"

import axios from "axios"
import { useDispatch } from 'react-redux';
import { cancelAppointment } from "../../redux/reducer";


const Turno = ({ date, time, status, id})=>{
    const dispatch = useDispatch() // para enviarle la info al reducer de turnos
    const cancelAppointmentQuery = async () => {
        try {
            const res = await axios.put(`http://localhost:3000/appointments/cancel/${id}`)
            console.log(res);
            dispatch(cancelAppointment(id))
            
        } catch (error) {
            console.log("Error obtaining appointments: ", error);
            
        }
    }

    const formatDate = (dateIso) => {
        const dateObject = new Date(dateIso);
        return dateObject.toLocaleDateString('en-GB');  // Utilizando 'en-GB' para obtener el formato DD/MM/YYYY
    };

    const handleCancel = ()=>{
        if (window.confirm("Are you sure you want to cancel this appointment?")) {
            cancelAppointmentQuery();
        } 
    }

    return(
        <div className={styles.card}>
            <img className={styles.image} src={logoTurn} alt="logoTurn"/>
            <div className={styles.text}>
                <p>SAVE THE DATE</p>
                <p>You are going to surpass your limits on: {formatDate(date)}</p>
                <p>Arrival time: {time} </p>
                
            </div>
            <div>
                <button disabled={(new Date(date) <= new Date()) || status === "cancelled"} onClick={handleCancel} className={styles.btn}>
                    <svg viewBox="0 0 15 17.5" height="17.5" width="15" xmlns="http://www.w3.org/2000/svg" className={styles.icon}>
                    <path transform="translate(-2.5 -1.25)" d="M15,18.75H5A1.251,1.251,0,0,1,3.75,17.5V5H2.5V3.75h15V5H16.25V17.5A1.251,1.251,0,0,1,15,18.75ZM5,5V17.5H15V5Zm7.5,10H11.25V7.5H12.5V15ZM8.75,15H7.5V7.5H8.75V15ZM12.5,2.5h-5V1.25h5V2.5Z" id="Fill"></path>
                    </svg>
                </button>
                <p className={[(status=== "active") ? styles.turnStateActive: styles.turnStateCancelled]}> {status}</p>
            </div>
            
        </div>
    )
}
export default Turno